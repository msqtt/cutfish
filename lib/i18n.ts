...
const resources = {
  en: {
    translation: {
      ...
      tts_export_voice: 'Export voice', tts_include_export: 'Include in export',
      tts_instant_preview: 'Instant preview (browser)',
      tts_local_preview: 'Preview (export voice)',
      tts_generating: 'Generating…', tts_downloading_model: 'Downloading model…',
      tts_progress: '{{percent}}%',
      tts_model_info: 'Model: {{size}} MB · First use downloads from external CDN, inference runs locally · Cached in browser OPFS',
      tts_media_private: 'Subtitle text and media are never uploaded.',
      tts_generation_failed: 'TTS generation failed. Please try again.',
      tts_export_stage: 'Generating TTS audio…',
      tts_export_progress: 'TTS {{current}}/{{total}}',
      tts_voice_group_zh: 'Chinese', tts_voice_group_en: 'English',
      tts_no_voices: 'No voices available in this browser.',
      tts_browser_unsupported: 'Browser speech unavailable — instant auto-preview disabled; local VITS export still works.',
      // Visual overlays
  ...
  zh: {
    translation: {
      ...
      tts_export_voice: '导出音色', tts_include_export: '包含在导出中',
      tts_instant_preview: '即时预览（浏览器）',
      tts_local_preview: '试听（导出音色）',
      tts_generating: '正在生成…', tts_downloading_model: '正在下载模型…',
      tts_progress: '{{percent}}%',
      tts_model_info: '模型：{{size}} MB · 首次使用从外部 CDN 下载，推理完全本地 · 缓存在浏览器 OPFS',
      tts_media_private: '字幕文本和媒体不会被上传。',
      tts_generation_failed: 'TTS 生成失败，请重试。',
      tts_export_stage: '正在生成 TTS 音频…',
      tts_export_progress: 'TTS {{current}}/{{total}}',
      tts_voice_group_zh: '中文', tts_voice_group_en: '英文',
      tts_no_voices: '当前浏览器无可用语音。',
      tts_browser_unsupported: '浏览器语音不可用——即时自动预览已禁用，本地 VITS 导出仍可正常使用。',
      // Visual overlays
