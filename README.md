...
- **Content-driven multitrack timeline** — V1 Video and A1 Source audio are a linked pair: dragging either row with Pointer Events reorders both, with visible handles and Alt+Arrow fallback, and clicking either seeks to the same source/project time. A2 Music & audio, A3 Voice / TTS, S1 Captions, I1 Images, and FX Visual effects appear only when material exists. Pointer drags start continuous editing only past a 6px threshold, so a click/tap selects without opening a history checkpoint; A2 and timed items clamp inside the project and preserve state identity on boundary no-ops. Selection highlights the exact lane with a strong `--selection` ring and its linked counterpart (V1↔A1, A3↔S1) with a secondary `--linked-selection` ring, with consistent `aria-pressed`; A1 and linked A3/S1 items show a Link2 indicator while fixed filters/transitions show a Lock indicator and “fixed, not movable” description. The toolbar shows a truthful “Project timeline” title with a derived visible-track count and a native (keyboard-accessible, non-ARIA-menu) Add disclosure for Caption, Text, and Image that reuses the existing creation paths at the playhead — revealing the Inspector only on compact layouts and closing after the action; audio still enters through Media Assets. Selection drives contextual Inspector settings, and Cut/Delete act on the selected material at the playhead
