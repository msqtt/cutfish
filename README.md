...
- **Editable background audio track (A2)** — a background-audio source plus independently editable timeline segments, each with its own project start, source trim (start/end), volume, and fade-in/out; import or replace the audio source, split a segment at the project playhead, and delete segments. Legacy single-background-music drafts migrate automatically to one segment, and tracks imported without duration metadata are hydrated from the local file. File is persisted in IndexedDB (restored with a tracked URL on project switch). Choose normal mixing or replace all video-source audio while keeping subtitle TTS
...
- **Content-driven multitrack timeline** — V1 video and its speed-aligned A1 source-audio blocks are always paired. A2 background audio, S1 subtitles, I1 images, and FX effects appear only while those elements exist, so empty rows never occupy the editor. Video remains reorderable; background audio and timed overlays support selection, drag movement, and keyboard nudging (0.1s, or 1s with Shift). The panel is resizable, collapsible, and maximizable, and shows editing + output duration when transitions reduce total
