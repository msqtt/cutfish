...
- **统一素材导入**：视频、音频、图片均从媒体资源入口导入，支持拖放、视频重复检测与进度追踪
...
- **可听的时间轴 TTS 旁白**：对非空字幕启用 TTS 后自动出现 A3 旁白块并预生成本地 Piper WAV；播放进入 cue 时通过已解锁的 Web Audio 引擎播放与导出一致的缓存/生成音色，seek、暂停、切项目或卸载会取消过期请求
...
- **可编辑背景音频轨（A2）**：从媒体资源导入音频，可在播放头处添加，或拖到 A2 的指定项目时间；每段独立设置项目起点、源裁剪、音量和淡入淡出，并在时间轴播放时真实可听。替换视频原声模式在预览中也会静音 A1；导入新音源前会确认。旧草稿自动迁移，File 持久化于 IndexedDB
...
- **素材驱动的动态多轨时间轴**：V1「视频」与 A1「视频原声」为联动配对，拖动任一轨道都会通过 Pointer Events 重排整对素材，并提供可见把手和 Alt+方向键回退；点击任一轨道都会定位到相同的素材/项目时间。A2「音乐与音频」、A3「语音 / TTS」、S1「字幕」、I1「图片」和 FX「视觉效果」仅在存在素材时出现。指针移动超过 6px 后才开启连续拖动，因此点击/轻触只负责选择，不产生空历史记录；A2 与定时元素始终限制在项目范围内，边界无操作保持状态引用不变。选择使用 `--selection` 强调精确轨道，并以 `--linked-selection` 次级强调 V1↔A1、A3↔S1 的关联轨；A1 和关联 A3/S1 显示 Link2 标识，固定滤镜/转场显示 Lock 和“固定、不可移动”说明。工具栏显示真实的“项目时间轴”标题和动态可见轨道数，并提供原生键盘可访问的「添加」折叠入口，可在播放头处新增字幕、文字和图片；紧凑布局仅在明确新增时打开检查器，音频仍从媒体资源进入。选择驱动上下文检查器，Cut/Delete 作用于播放头处所选素材
- **上下文检查器**：选择 V1、A1、A2、A3/S1、I1 或 FX 后优先展示对应的视频、原声、音频段、字幕/TTS、标注或效果设置，隐藏无关上下文标签，同时保持项目/全局入口稳定；移动端选择不会打断时间轴操作，检查器由顶栏显式打开
