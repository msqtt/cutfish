...
- Exportable local TTS: each subtitle can select a Piper/VITS voice, enable/disable export mixing, set rate and volume, preview the exact generated WAV, and burn speech into the final audio track at cue project time; enabling a non-empty cue derives an A3 block and pre-generates the same local WAV used by timeline playback and export; an unlocked Web Audio context plus request tokens prevent autoplay rejection and stale post-seek playback; models (20–65 MB) are validated through a same-origin mirror with configured/official fallbacks, cached and automatically repaired in OPFS, and all inference remains local; in-memory Map cache keyed by voice+text+rate is reused; export generates WAVs before FFmpeg, writes them to MEMFS, and fails visibly on generation errors
...
Imported background audio is represented by one source `File` plus independently editable timeline segments. Audio is imported through Media Assets and can be added at the playhead or dragged onto A2 at a specific project time; importing a new source requires confirmation because the compatibility model currently keeps one source. Each segment stores `id`, project-relative `projectStart`, source-relative `trimStart`/`trimEnd`, volume, fade-in, and fade-out. A segment can be selected, moved, trimmed, split at the project playhead, or deleted. A project-clock-driven HTML audio element previews trim, gain/fades, master volume and playback speed; replacement mode mutes A1 during preview.
...
TTS export uses pinned `@diffusionstudio/vits-web@1.0.3` (MIT) with Piper ONNX voices. The package returns a WAV `Blob` entirely in-browser. Because the upstream package neither checks HTTP status nor validates payloads, Cutfish preloads each curated model through a same-origin `/tts-models/*` mirror proxy with configured/official fallbacks, validates the ONNX size/content type and Piper JSON schema, then writes the pair to the package-compatible `piper` OPFS directory (model last). Missing, partial, or corrupt cached pairs are repaired automatically; a previously cached model that fails inference is refreshed once. Subsequent generation is local. Static model/runtime requests never contain subtitle text or media. Curated choices include Chinese Huayan fast/quality models and multiple English female/male voices.

Manual preview and timeline auto-preview synthesize the exact selected Piper voice, reusing an in-memory WAV cache keyed by voice+text+rate. Enabling TTS on non-empty text pre-generates the WAV and derives a dynamic A3 block linked to the subtitle cue. The Play gesture resumes a persistent Web Audio context; generated WAV buffers can therefore start after asynchronous model inference without relying on a new HTMLAudio autoplay grant. Request tokens and buffer-source cleanup prevent stale playback after seek, pause, project switch, or teardown. Rate and volume match export settings, and project master volume applies to A1, A2, and TTS in both preview and FFmpeg export. UI exposes model progress, include-in-export, model size, OPFS caching, and the local privacy notice.
